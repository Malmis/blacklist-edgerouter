...
for url in "${LIST_URLS[@]}"; do
  ...
  grep -Eo '([0-9]{1,3}\.){3}[0-9]{1,3}/([0-9]|[12][0-9]|3[0-2])' "$f" >> "$ALL_CIDR" || true
done
...
tr -d '\r' < "$ALL_CIDR" | validate_cidr | filter_reserved_cidr | sort -u > "$FILTERED"

# Whitelist
if [[ "${#WHITELIST_CIDR[@]}" -gt 0 ]]; then
  WL="$WORK/whitelist_cidr.txt"
  printf "%s\n" "${WHITELIST_CIDR[@]}" | tr -d '\r' | validate_cidr | sort -u > "$WL"
...
tr -d '\r' < "$FINAL" | sed 's/[[:space:]]\+$//' | awk 'length' | sort -u > "$WORK/final_cidr.norm"
...
if [[ "$DELN" -gt 0 ]]; then
  ...
  while IFS= read -r cidr; do
    [[ -n "$cidr" ]] || continue
    if $OP show configuration commands \
          | grep -F -x "set firewall group network-group ${GROUP_NET} network ${cidr}" >/dev/null; then
...
$OP show configuration commands \
  | grep -F "set firewall group network-group ${GROUP_NET} network " \
  | awk '{ $1=$1; print }' | sort -u > "$POST_SET" || true
POST_COUNT=$(sed -E "s|^set firewall group network-group ${GROUP_NET} network ||" "$POST_SET" \
               | awk 'length' | wc -l)
